...
try %So that if something fails, communications are closed properly
    ...
    inout1 = 0;
    inout2 = 0;
    pad = 20; %this is the minimum number of samples two events can be apart
    str_num = [];
    ...
    while ~STOP %only runs if stop button is not pressed
        ...
        if framenum.Value~= datlog.framenumbers.data(frameind.Value,1) %Frame num value changed, reading data
            ...
            for l=1
                ...
                %aux=(datevec(now)-datevec(lastRead));
                LAnk_diff(frameind.Value) = LANK_pos(2) - RANK_pos(2);
                RAnk_diff(frameind.Value) = RANK_pos(2) - LANK_pos(2);
                
                LAnk_diffdiff(frameind.Value) = LAnk_diff(frameind.Value)-LAnk_diff(frameind.Value-1);
                %LAnk_diffdiff(LAnk_diffdiff==0) = [];
                %LAnk_diffdiff(isnan(LAnk_diffdiff)) = []
                %sign_LAnk_diff(frameind.Value) = sign(LAnk_diffdiff(frameind.Value));
                
                %if sign(LAnk_diffdiff(frameind.Value)) ~= sign(LAnk_diffdiff(frameind.Value-1))
                    if LAnk_diffdiff(frameind.Value) >= 0 && sign(LAnk_diffdiff(frameind.Value)) ~= sign(LAnk_diffdiff(frameind.Value-1))
                        Left_HS(frameind.Value) = 1;
                        sum(Left_HS)
                    elseif LAnk_diffdiff(frameind.Value) < 0 && sign(LAnk_diffdiff(frameind.Value)) ~= sign(LAnk_diffdiff(frameind.Value-1))
