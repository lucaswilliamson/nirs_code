...
try %So that if something fails, communications are closed properly
    ...
    if ~nirs
        ...
        %Oxysoft.WriteEvent('I','Connected'); %FIXME: uncomment
        %ss = serial('COM13');
...
%         eventorder_all = [0,1,2; 2, 1, 0; 1,2,0; 1,0,2; 0,2,1; 2,0,1];%TODO: to be randomized for each participant
        eventorder_all = [0,1,2; 2, 1, 0];
        trialIndex = 1;
        eventorder = eventorder_all(trialIndex,:); %0 = stand and alphabet, 1 = walk and alphabet, 2 = walk, 
        %set up audio players
        if alphabetLetter == 'A'
            audioFileNames = {'walk','rest','standAndA','stopAndRest','turnAndStop','walkAndA'};
        else
            audioFileNames = {'walk','rest','standAndB','stopAndRest','turnAndStop','walkAndB'};
        end
        audioids = {'walk','rest','standAlphabet','stopAndRest','turnAndStop','walkAlphabet'};
        instructions = containers.Map();
        ...
        %fopen(ss);fclose(ss);
%         play(instructions('rest'));
%         datlog.audioCues.start(end+1)=now;
%         datlog.audioCues.instructions(end+1) = 'rest';
        %Oxysoft.WriteEvent('R','Rest'); %FIXME: uncomment
        datlog = nirsEvent('rest', 'R', 'Rest', instructions, datlog, Oxysoft);
        pause(restDuration);
%         datlog.audioCues.stop(end+1) = now; %TODO: save frames too

        currentIndex = 1;
        if eventorder(currentIndex)== 0 %stand and alphabet
            datlog = nirsEvent('standAlphabet', 'A', ['Stand and Alphabet with ' alphabetLetter], instructions, datlog, Oxysoft);
%             disp('StandAlphabet')
%     %         fopen(ss);fclose(ss);
%             play(instructions('standAlphabet'));
%             datlog.audioCues.start(end+1)=now;
%             datlog.audioCues.instructions(end+1) = 'standAlphabet';
    %         Oxysoft.WriteEvent('A','Stand and Alphabet') %FIXME: uncomment                        
            pause(restDuration)
%             datlog.audioCues.stop(end+1) = now;

            %complete follow a rest block
    %         fopen(ss);fclose(ss);
%             play(instructions('stopAndRest'));
    %         Oxysoft.WriteEvent('R','Rest'); %FIXME: uncomment
            datlog = nirsEvent('stopAndRest','R','Rest', instructions, datlog, Oxysoft);
            pause(restDuration);
...
%             disp('Walk and alphabet')
    %         fopen(ss);fclose(ss);
%             play(instructions('walkAlphabet'));
    %         Oxysoft.WriteEvent('B','Walk and Alphabet') %FIXME: uncomment
            datlog = nirsEvent('walkAlphabet','B',['Walk and Alphabet' alphabetLetter], instructions, datlog, Oxysoft);
            currentIndex  = currentIndex + 1;
        elseif eventorder(currentIndex) == 2 %firstw event is walk
%             disp('Walk')
    %         fopen(ss);fclose(ss);
%             play(instructions('walk'));
    %         Oxysoft.WriteEvent('W','Walk');  %FIXME: uncomment
            datlog = nirsEvent('walk','W','Walk', instructions, datlog, Oxysoft);
            currentIndex = currentIndex + 1;
        end
    end
    ...
    %TODO: Shuqi
    if nirs
        y_max = 4000;%0 treadmill, max = 7 tile = 14 ft ~= 4.2672meter
        y_min = -1450;%0 treadmill, min = 2.3 tile = 4.6 ft ~= 1.4021 meter
    end
...
%                             Speak(obj, ' Good Job');
                ...
                if nirs && inout1 == 1 && inout2 == 1 %TODO: Shuqi   
                    ...
                    inout1 = 0; %reset
                    inout2 = 0;
                    %TODO: change to only play turn around after a full
                    %round.
%                         fopen(ss);fclose(ss);
%                     play(instructions('turnAndStop'));
%                         Oxysoft.WriteEvent('R','Rest'); %FIXME: to
%                         uncomment
                    datlog = nirsEvent('turnAndStop','R','Rest', instructions, datlog, Oxysoft);
                    pause(restDuration);
%                     datlog.audioCues.stop(end+1) = now;

                    if currentIndex > length (eventorder)
                        %TODO: should automatically send signal to
                        %terminate the software
%                         fprintf('Task end. Current order: %d %d %d', eventorder) 
%                             fopen(ss);fclose(ss);
                        Speak(obj, 'Trial End') %TODO: in actual test, pause or say stop and rest
%                             Oxysoft.WriteEvent('E','End'); %FIXME: to
%                             uncomment
                        datlog = nirsEvent('','E','Trial End', instructions, datlog, Oxysoft); %no audio
                        ...
                        if (trialIndex >  size(eventorder_all,1))
                            ...
                            datlog = nirsEvent('','O','Experiment End', instructions, datlog, Oxysoft); %no audio
                        else  %Restart a trial
                            eventorder = eventorder_all(trialIndex,:); %TODO: to be randomized for each participant
...
%                             play(instructions('standAlphabet'));
                %         Oxysoft.WriteEvent('A','Stand and Alphabet') %FIXME: uncomment                        
                        datlog = nirsEvent('standAlphabet','A',['Stand and Alphabet' alphabetLetter], instructions, datlog, Oxysoft);
                        pause(restDuration)
