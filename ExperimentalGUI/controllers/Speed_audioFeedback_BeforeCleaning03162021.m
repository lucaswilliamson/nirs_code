...
if ~nirs
    ...
    disp('Initial Setup')
    %TODO: change to play audio instead of speak
    ...
    eventorder_all = [0,1,2; 1,2,0; 1,0,2; 2,1,0; 0,2,1; 2,0,1];%TODO: to be randomized for each participant
    trialIndex = 1;
...
%     play(walk_instruction);
%     stop(walk_instruction);
...
try %So that if something fails, communications are closed properly
    ...
    % Added by Yashar %TODO: Shuqi Relocated
%     NET.addAssembly('System.Speech');
%     obj = System.Speech.Synthesis.SpeechSynthesizer;
...
%                             uncomment
                        ...
                        if eventorder(currentIndex)== 0 %stand and alphabet
                            disp('StandAlphabet')
                    %         fopen(ss);fclose(ss);
                            play(instructions('stabdAlphabet'));
                    %         Oxysoft.WriteEvent('A','Stand and Alphabet') %FIXME: uncomment                        
                            pause(restDuration)
